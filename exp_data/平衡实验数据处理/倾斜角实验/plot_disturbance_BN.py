...
ppo_data = pd.read_csv('roll_angle_ppo_BN.csv', encoding='gbk')
...
ppo_data = smooth_region(715 - left,  725 - left, ppo_data, window_length=9, polyorder=5, mode='mirror')


ppo_data[4:] = ppo_data[:-4]
ppo_data[:102] = 0

pid_data = pd.read_csv('roll_angle_pid_BN.csv', encoding='gbk')
...
# 显示网格
plt.grid(True)
plt.savefig('扰动图_BN_宋体_字体变大.png', format='png', dpi=600)
# 显示图形
plt.show()
