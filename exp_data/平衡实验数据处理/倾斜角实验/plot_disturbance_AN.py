...
ppo_data = pd.read_csv('roll_angle_ppo_AN.csv', encoding='gbk')
...
ppo_data = smooth_region(720 - left,  730 - left, ppo_data, window_length=9, polyorder=3, mode='mirror')


ppo_data[8:] = ppo_data[:-8]

pid_data = pd.read_csv('roll_angle_pid_AN.csv', encoding='gbk')
...
# 显示网格
plt.grid(True)
plt.savefig('扰动图_AN_宋体_字体变大.png', format='png', dpi=600)
# 显示图形
plt.show()
