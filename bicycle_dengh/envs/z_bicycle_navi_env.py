...
class ZBicycleNaviEnv(gymnasium.Env):
    ...
    def __init__(self, gui=False):
        ...
        # 车把角度，前后轮速度, 飞轮速度, 相对路径点的距离x, 相对路径点的距离y
        self.actual_action_space = gymnasium.spaces.box.Box(
            low=np.array([-1.57, 0., -self.max_flywheel_vel, -5.0, -5]),
            high=np.array([1.57, 5., self.max_flywheel_vel, 5.0, 5]),
            shape=(5,),
            dtype=np.float32)
    ...
    def step(self, action):
        ...
        ref_x_y = rescaled_action[3:]
        self.bicycle.apply_action(rescaled_action[:3])
        p.stepSimulation(physicsClientId=self.client)
        self.goal_temp = self.goal_temp + ref_x_y
    ...
    def _reward_fun(self, obs, lidar_data):
        self.terminated = False
        self.truncated = False
        ...
        total_reward = balance_rwd + navigation_rwd + still_penalty
